#!/usr/bin/env python3
"""scratch_guard.py — single implementation of the .codex/tmp/ containment check.

Usage: python3 scratch_guard.py [-p|-w] <path>

Prints the normalized repo-relative path on stdout and exits 0 when <path>
stays under .codex/tmp/. Exits 1 on any violation (message on stderr).
...
resolves inside .codex/tmp/ (an ancestor resolving outside exits 1, so a
...
cannot redirect a file write outside .codex/tmp/; use it for write targets.
"""
